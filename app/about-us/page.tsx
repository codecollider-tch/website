...
export const metadata = {
  ...
  description: 'Learn more about Code Collider, your partner in business growth through comprehensive website consulting, competitive analysis, and data-driven digital strategies.',
};
...
export default async function AboutPage() {
  ...
  const teamMembers = (teamResponse.data.teamConnection.edges?.map((edge) => edge?.node).filter((node): node is NonNullable<typeof node> => node != null) || []);

  // Fetch FAQ items
  const faqResponse = await client.queries.faqConnection();
  const faqs = (faqResponse.data.faqConnection.edges?.map((edge) => edge?.node).filter((node): node is NonNullable<typeof node> => node != null) || []);
    ...
    <>
      ...
      <main className="page-wrapper">
        ...
        <AboutExpertise
          ...
          expertise={expertise?.filter((item): item is NonNullable<typeof item> => item != null) || []}
        />
        <TeamMembers members={teamMembers} />
        <BusinessLevelStats stats={stats?.filter((item): item is NonNullable<typeof item> => item != null) || []} />
